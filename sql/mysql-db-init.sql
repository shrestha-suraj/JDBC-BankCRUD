...
SELECT * FROM Transaction INNER JOIN Account ON Transaction.account_id=Account.id INNER JOIN Bank ON Account.bank_id=Bank.id INNER JOIN Patron ON Account.patron_id=Patron.id WHERE Transaction.id=2;
